...
(function init($, window, document, undefined) {
  ...
  class SlotMachine {
    ...
    _initFakeTiles() {
      ...
      this._$fakeFirstTile = this.$tiles.eq(this.$tiles.length - 2).clone();
      this._$fakeLastTile = this.$tiles.eq(1).clone();

      // Add fake titles to the DOM
      this.$container.prepend(this.$tiles.last().clone());
      this.$container.prepend(this._$fakeFirstTile);
      this.$container.append(this.$tiles.first().clone());
    ...
    getTileOffset(index) {
      ...
      for (let i = 0; i < index; i++) {
        ...
        simpleOne = this.$tiles.eq(i).outerHeight();
      }

      // console.log("noew" + (this._minTop - offset));
      // offset -= simpleOne;
      // console.log("after" + (this._minTop - offset));
